...
class Data:
    ...
    def prepare_image(self, object_based_coloring: bool = False):
        ...
        areas = [{"id": obj["id"], "area": obj["area"]} for obj in objects]
        ...
        return full_path, objects, names_colors, img_obj_categories, obj_ids, img_categories, areas
    ...
    def goto_image(self, n):
        """Loads an arbitrary image in a list"""
        self.current_image = self.images.goto(n)
...
def get_areas(instances: dict) -> dict:
    return dict([[annotation["id"], annotation["area"]] for annotation in instances['annotations']])
...
class StatusBar(ttk.Frame):
    ...
    def __init__(self, parent):
        ...
        self.file_count = ttk.Entry(self, background="gray75")
...
class Menu(tk.Menu):
    ...
    def file_menu(self):
        ...
        menu.add_command(label="Reload", accelerator="Ctrl+R")
...
class Controller:
    def __init__(self, data, root, image_panel, statusbar, menu, objects_panel, sliders):
        ...
        self.setup_menu()
        self.init_local_setup()
        self.object_panel_setup()
        self.slider_setup()
        self.init_image()
        self.bind_events()

    def reset(self, event=None):
        self.data = Data(self.data.image_dir, self.data.annotations_file)
        self.setup_statusbar()
        self.setup_menu()
        self.init_local_setup()
        self.object_panel_setup()
    ...
    def setup_menu(self):
        ...
        self.menu.file.entryconfigure("Reload", command=self.reset)
    ...
    def update_img(self, local=True, width=None, alpha=None, label_size=None):
        ...
        self.current_img_obj_ids = img_obj_ids
        self.current_img_obj_areas = img_obj_areas
    ...
    def goto_img(self, event=None):
        n = int(self.file_count_status.get().split('/')[0].strip())
        self.data.goto_image(n)
        self.set_locals()
        self.selected_cats = None
        self.selected_objs = None
        self.update_img(local=False)
    ...
    def update_object_box(self):
        ...
        obj_ids = self.current_img_obj_ids
        names = [self.data.categories[i][0] for i in ids]
        areas = [area['area'] for area in self.current_img_obj_areas]
        self.object_box_content.set([" ".join([str(i), str(n), f"- {a}"]) for i, n, a in zip(ids, names, areas)])
    ...
    def bind_events(self):
        ...
        self.statusbar.file_count.bind("<Return>", self.goto_img)
