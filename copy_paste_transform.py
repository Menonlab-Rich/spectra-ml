...
class CopyPaste(A.DualTransform):
    ...
    def get_params_dependent_on_data(self, params, data):
        image = data["paste_image"]
        masks = None
        if "paste_mask" in data:
            #handle a single segmentation mask with
            #multiple targets
            #nothing for now.
            raise NotImplementedError
        elif "paste_masks" in data:
            masks = data["paste_masks"]

        assert(masks is not None), "Masks cannot be None!"

        bboxes = data.get("paste_bboxes", None)
        keypoints = data.get("paste_keypoints", None)
    ...
    def apply_to_targets(self, targets, **params):
            # The `targets` dictionary contains all the data passed to the transform
            # (e.g., 'image', 'masks')
            targets['image'] = self.apply(targets['image'], **params)
            targets['masks'] = self.apply_to_masks(targets['masks'], **params)
            return targets
